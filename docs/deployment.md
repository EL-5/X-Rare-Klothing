...
Batch 26: the app is installable to a phone's home screen (or desktop) via `vite-plugin-pwa` — `vite.config.ts` configures a web manifest (`X-Rare`, standalone display, icons, theme/background colors matching `--color-ink`/`--color-surface`) and Workbox generates a service worker (`dist/sw.js`) that precaches only the built app shell. **Deliberately no runtime-caching rule for the Supabase origin** — this is a live storefront with real prices/stock/cart state, so every API call always hits the network; only static JS/CSS/HTML/icons are cached.

Icons (`public/pwa-192.png`, `public/pwa-512.png`, `public/maskable-icon-512.png`, `public/apple-touch-icon.png`) are generated from the existing `public/favicon.png` logo mark via `npm run icons` (uses `sharp`, a devDependency) — re-run it if the logo ever changes; the generated PNGs are checked into `public/` since they're static assets, not build output.

Installability itself couldn't be verified with a real "Add to Home Screen" prompt in this development sandbox — service worker registration fails there for any script, including a trivial unrelated test worker, which points to the sandbox blocking persistent SW registration rather than anything wrong with the app. Confirmed instead by inspecting the built output directly: `dist/manifest.webmanifest` parses with the correct fields, `dist/sw.js` serves with the correct MIME type, and `index.html` has the manifest `<link>`, `registerSW.js` script tag, `apple-touch-icon` link, and `theme-color` meta all correctly injected. This should register and prompt normally in any real browser once deployed over HTTPS.
...
| PWA / installable | ✅ Batch 26 — manifest + service worker verified via build output; live install-prompt untestable in this sandbox (SW registration blocked there for any script) |
