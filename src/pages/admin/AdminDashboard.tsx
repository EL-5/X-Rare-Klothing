...
import { DollarSign, ShoppingCart, Receipt, Users, Package, AlertTriangle, XCircle, Clock, CreditCard } from 'lucide-react';
  ...
  return (
    <div>
      ...
      {error ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-4 lg:grid-cols-3 xl:grid-cols-5">
            <StatCard label="Revenue" value={formatMoney(data.metrics.revenue)} icon={DollarSign} />
            <StatCard label="Orders" value={String(data.metrics.ordersCount)} icon={ShoppingCart} />
            <StatCard label="Avg. order value" value={formatMoney(data.metrics.averageOrderValue)} icon={Receipt} />
            <StatCard label="New customers" value={String(data.metrics.newCustomers)} icon={Users} />
            <StatCard label="Products sold" value={String(data.metrics.productsSold)} icon={Package} />
            <StatCard
              label="Low stock"
