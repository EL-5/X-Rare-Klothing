...
export function ProductInformation({ product, price, compareAtPrice, sku }: ProductInformationProps) {
  ...
  return (
    <div>
      {product.brand ? <p className="text-xs uppercase tracking-wide text-ink/50">{product.brand}</p> : null}
      <h1 className="mt-1 text-xl font-semibold uppercase tracking-wide text-ink lg:text-2xl">{product.title}</h1>
      <p className="mt-3 text-lg text-ink">
        {isOnSale ? <span className="mr-2 text-ink/40 line-through">{formatMoney(compareAtPrice)}</span> : null}
        {formatMoney(price)}
      </p>
      {sku ? <p className="mt-1 text-xs text-ink/40">SKU: {sku}</p> : null}
    </div>
  );
}
